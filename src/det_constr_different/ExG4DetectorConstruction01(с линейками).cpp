...
{  
  ...
  G4double tgt_sizeX = 4*cm, tgt_sizeZ = 3.0*cm, tgt_sizeY = 5*cm;
  G4double lin_sizeX = 4*cm, lin_sizeY=1.5*cm, lin_sizeZ = 0.16*mm;
  ...
  G4Material* lin_mat = nist->FindOrBuildMaterial("G4_C"); 
...
/// Коллиматор
 ...
 G4Box* solidTgt0 =
  ...
  G4Box* lin1 =
    new G4Box("lin1",                    //its name, имя
      0.5*lin_sizeX, 0.5*lin_sizeY, 0.5*lin_sizeZ); //its size, размеры

  //Логический объем
  G4LogicalVolume* logicLin1 =
    new G4LogicalVolume(lin1,            //its solid, объем
                        lin_mat,             //its material, указываем материал детектора
                        "logicLin1");         //its name, его имя

  //Физический объем детектора
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,6.2*cm),         //at (0,0,0)
                    logicLin1,                //its logical volume, подключаем логический объем
                    "physLin1",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                 // checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube1_1 =
    new G4Tubs("Tube1_1",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube1_1 =
    new G4LogicalVolume(Tube1_1,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube1_1", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , 2.5*mm ,6.25*cm),         //at (0,0,0)
                    logicTube1_1,                //its logical volume, подключаем логический объем
                    "physTube1_1",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                  //  checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube1_2 =
    new G4Tubs("Tube1_2",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube1_2 =
    new G4LogicalVolume(Tube1_2,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube1_2", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , -2.5*mm ,6.25*cm),         //at (0,0,0)
                    logicTube1_2,                //its logical volume, подключаем логический объем
                    "physTube1_2",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                    //checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4Box* lin2 =
    new G4Box("lin2",                    //its name, имя
      0.5*lin_sizeX, 0.5*lin_sizeY, 0.5*lin_sizeZ); //its size, размеры

  //Логический объем
  G4LogicalVolume* logicLin2 =
    new G4LogicalVolume(lin2,            //its solid, объем
                        lin_mat,             //its material, указываем материал детектора
                        "logicLin2");         //its name, его имя

  //Физический объем детектора
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,11.7*cm),         //at (0,0,0)
                    logicLin2,                //its logical volume, подключаем логический объем
                    "physLin2",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                   // checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube2_1 =
    new G4Tubs("Tube2_1",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube2_1 =
    new G4LogicalVolume(Tube2_1,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube2_1", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , 2.5*mm ,11.75*cm),         //at (0,0,0)
                    logicTube2_1,                //its logical volume, подключаем логический объем
                    "physTube2_1",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                   // checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube2_2 =
    new G4Tubs("Tube2_2",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube2_2 =
    new G4LogicalVolume(Tube2_2,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube2_2", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , -2.5*mm ,11.75*cm),         //at (0,0,0)
                    logicTube2_2,                //its logical volume, подключаем логический объем
                    "physTube2_2",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                   // checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
...
//  
 ...
 G4Box* lin3 =
    new G4Box("lin3",                    //its name, имя
      0.5*lin_sizeX, 0.5*lin_sizeY, 0.5*lin_sizeZ); //its size, размеры

  //Логический объем
  G4LogicalVolume* logicLin3 =
    new G4LogicalVolume(lin3,            //its solid, объем
                        lin_mat,             //its material, указываем материал детектора
                        "logicLin3");         //its name, его имя

  //Физический объем детектора
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,17.2*cm),         //at (0,0,0)
                    logicLin3,                //its logical volume, подключаем логический объем
                    "physLin3",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                   // checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube3_1 =
    new G4Tubs("Tube3_1",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube3_1 =
    new G4LogicalVolume(Tube3_1,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube3_1", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , 2.5*mm ,17.25*cm),         //at (0,0,0)
                    logicTube3_1,                //its logical volume, подключаем логический объем
                    "physTube3_1",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                  //  checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube3_2 =
    new G4Tubs("Tube3_2",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube3_2 =
    new G4LogicalVolume(Tube3_2,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube3_2", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , -2.5*mm ,17.25*cm),         //at (0,0,0)
                    logicTube3_2,                //its logical volume, подключаем логический объем
                    "physTube3_2",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                  //  checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
...
//  
 ...
 G4Box* lin4 =
    new G4Box("lin4",                    //its name, имя
      0.5*lin_sizeX, 0.5*lin_sizeY, 0.5*lin_sizeZ); //its size, размеры

  //Логический объем
  G4LogicalVolume* logicLin4 =
    new G4LogicalVolume(lin4,            //its solid, объем
                        lin_mat,             //its material, указываем материал детектора
                        "logicLin4");         //its name, его имя

  //Физический объем детектора
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,22.7*cm),         //at (0,0,0)
                    logicLin4,                //its logical volume, подключаем логический объем
                    "physLin4",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                   // checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube4_1 =
    new G4Tubs("Tube4_1",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube4_1 =
    new G4LogicalVolume(Tube4_1,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube4_1", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , 2.5*mm ,22.75*cm),         //at (0,0,0)
                    logicTube4_1,                //its logical volume, подключаем логический объем
                    "physTube4_1",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                  //  checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
  ...
  G4VSolid* Tube4_2 =
    new G4Tubs("Tube4_2",                    //its name, имя
         0.*mm, 0.5*1.2*mm, 0.5*4*cm, 0., tube_dPhi*rad); //its size, размеры

  G4LogicalVolume* logicTube4_2 =
    new G4LogicalVolume(Tube4_2,            //its solid, объем
                        tube_mat,             //its material, указываем материал детектора
                        "logicTube4_2", 0, 0, 0);         //its name, его имя
   
 new G4PVPlacement(yRot,                       //no rotation, так же без вращения
                    G4ThreeVector(0 , -2.5*mm ,22.75*cm),         //at (0,0,0)
                    logicTube4_2,                //its logical volume, подключаем логический объем
                    "physTube4_2",              //its name, имя физического объема
                    logicWorld,              //its mother  volume, родительский логический объем!
                    false,                   //no boolean operation, без булевых операций
                    0);                       //copy number, номер копии
                  //  checkOverlaps);          //overlaps checking, флаг проверки перекрытия объемов
...
//1
   G4Box* solidTgt1 =
    new G4Box("Target1",                    //its name, имя
        0.5*tgt_sizeX, 0.5*tgt_sizeY, 0.5*tgt_sizeZ); //its size, размеры
  ...
  //Физический объем
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,8.75*cm),         //at (0,0,0)
...
//2
  ...
  //Физический объем
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,14.25*cm),         //at (0,0,0)
...
//3
  ...
  //Физический объем
  new G4PVPlacement(0,                       //no rotation, так же без вращения
                    G4ThreeVector(0,0,19.75*cm),         //at (0,0,0)
